...
# Defaults come from utils.sh (the single source of truth for NVECode branding).
. ./utils.sh

export CI_BUILD="${CI_BUILD:-no}"
export SHOULD_BUILD="${SHOULD_BUILD:-yes}"
export SKIP_ASSETS="${SKIP_ASSETS:-yes}"
export SKIP_BUILD="${SKIP_BUILD:-no}"
...
export MARKETPLACE_BACKEND="${MARKETPLACE_BACKEND:-vscode-marketplace}"

while getopts ":ilopsm:" opt; do
  case "$opt" in
    i)
      export ASSETS_REPOSITORY="${ORG_NAME}/nvecode-insiders"
      export BINARY_NAME="${BINARY_NAME}-insiders"
    ...
    s)
      ...
      ;;
    m)
      export MARKETPLACE_BACKEND="$OPTARG"
      ;;
    *)
      ;;
  esac
...
echo "MARKETPLACE_BACKEND=\"${MARKETPLACE_BACKEND}\""
...
if [[ "${SKIP_BUILD}" == "no" ]]; then
  if [[ "${SKIP_SOURCE}" != "no" ]]; then
    ...
    while [[ -n "$( git log -1 | grep "NVECODE HELPER" )" ]]; do
      git reset -q --hard HEAD~
    done
  ...
  if [[ -f "./include_${OS_NAME}.gypi" ]]; then
    ...
    if [[ -f "${HOME}/.gyp/include.gypi" ]]; then
      mv ~/.gyp/include.gypi ~/.gyp/include.gypi.pre-nvecode
    else
      echo "{}" > ~/.gyp/include.gypi.pre-nvecode
    fi
  ...
  if [[ -f "./include_${OS_NAME}.gypi" ]]; then
    mv ~/.gyp/include.gypi.pre-nvecode ~/.gyp/include.gypi
  fi
