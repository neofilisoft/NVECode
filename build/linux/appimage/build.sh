...
if [[ "${VSCODE_ARCH}" == "x64" ]]; then
  ...
  # add update's url
  ORG="${ORG_NAME:-Neofilisoft}"
  REPO="${APP_NAME_LC:-nvecode}"
  if [[ "${VSCODE_QUALITY}" == "insider" ]]; then
    sed -i "s/generate_type2_appimage/generate_type2_appimage -u \"gh-releases-zsync|${ORG}|${REPO}-insiders|latest|*.AppImage.zsync\"/" pkg2appimage.AppDir/AppRun
  else
    sed -i "s/generate_type2_appimage/generate_type2_appimage -u \"gh-releases-zsync|${ORG}|${REPO}|latest|*.AppImage.zsync\"/" pkg2appimage.AppDir/AppRun
  fi
  ...
  # workaround that enforces x86 ARCH for pkg2appimage; needed because pkg2appimage
  # inspects host arch from node_modules and may misclassify cross builds.
  export ARCH=x86_64
